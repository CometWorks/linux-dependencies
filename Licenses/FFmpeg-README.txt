...
The following shared libraries shipped alongside this notice are FFmpeg 8.1:
...
The exact build configuration used to produce the shipped binaries is in the
CometWorks/linux-dependencies repository at Scripts/build_ffmpeg.sh:

    https://github.com/CometWorks/linux-dependencies

The configure flags and the post-build patchelf step are reproduced verbatim
there.

Per the LGPL, users have the right to relink the application against a
modified FFmpeg of their choosing. Because the libraries are loaded via
dlopen at runtime, the user can simply replace the shipped .so files with
their own build (matching the same SOVERSIONs: avcodec 62, avformat 62,
avutil 60, swresample 6, swscale 9) and the application will pick them up
automatically.
